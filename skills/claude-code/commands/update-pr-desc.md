...
- **UT/BVT 覆盖说明**：当 PR 包含测试代码时，按函数分组用表格列出场景（输入/mock行为/期望结果），覆盖 happy path/error path/边界条件/并发安全；BVT 说明端到端场景和断言；Bug Fix 指出回归测试。无测试代码则不填
