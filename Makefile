...
BINARY ?= ai-cli
# TOOLS controls which tools are synced by skills targets.
TOOLS ?= all
# GOCACHE keeps go run/build cache out of restricted system paths.
GOCACHE ?= /tmp/gocache

.PHONY: help fmt test build run skills-install skills-upload
...
skills-install: ## Install skills/knowledge from repo to local tools (TOOLS=all|codex|kiro|claude-code)
	GOCACHE=$(GOCACHE) $(GO) run ./cmd/ai-cli skills --install --tools $(TOOLS)

skills-upload: ## Upload local skills/knowledge to repo and run diff-merge-commit-push (TOOLS=all|codex|kiro|claude-code)
